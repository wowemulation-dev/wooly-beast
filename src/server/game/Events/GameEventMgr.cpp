...
{
    {
        ...
        QueryResult result = WorldDatabase.Query("SELECT eventEntry, EXTRACT(EPOCH FROM start_time)::BIGINT, EXTRACT(EPOCH FROM end_time)::BIGINT, occurence, length, holiday, holidayStage, description, world_event, announce FROM game_event");
#else
