...
void WorldDatabaseConnection::DoPrepareStatements()
{
    ...
    PrepareStatement(WORLD_SEL_GAMEOBJECT_TARGET, "SELECT guid, id, position_x, position_y, position_z, map, (POWER(position_x - ?, 2) + POWER(position_y - ?, 2) + POWER(position_z - ?, 2)) AS order_ FROM gameobject WHERE map = ? AND id = ? ORDER BY order_ ASC LIMIT 1", CONNECTION_SYNCH);
...
WorldDatabaseConnection::WorldDatabaseConnection(PostgreSQLConnectionInfo& connInfo, ConnectionFlags connectionFlags) : PostgreSQLConnection(connInfo, connectionFlags)
{
}
