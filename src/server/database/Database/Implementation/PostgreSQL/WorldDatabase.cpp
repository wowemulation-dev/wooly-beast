...
PostgreSQLWorldDatabaseConnection::PostgreSQLWorldDatabaseConnection(PostgreSQLConnectionInfo& connInfo) : PostgreSQLConnection(connInfo)
{
}

PostgreSQLWorldDatabaseConnection::PostgreSQLWorldDatabaseConnection(ProducerConsumerQueue<SQLOperation*>* q, PostgreSQLConnectionInfo& connInfo) : PostgreSQLConnection(q, connInfo)
{
}

PostgreSQLWorldDatabaseConnection::~PostgreSQLWorldDatabaseConnection()
{
}

void PostgreSQLWorldDatabaseConnection::DoPrepareStatements()
{
    if (!m_reconnecting)
    ...
    PrepareStatement(WORLD_REP_LINKED_RESPAWN, "INSERT INTO linked_respawn (guid, linkedGuid, linkType) VALUES (?, ?, ?) ON CONFLICT (guid, linkType) DO UPDATE SET linkedGuid = EXCLUDED.linkedGuid", CONNECTION_ASYNC);
    ...
    // Convert POW() to POWER() for PostgreSQL
    PrepareStatement(WORLD_SEL_GAMEOBJECT_NEAREST, "SELECT guid, id, position_x, position_y, position_z, map, (POWER(position_x - ?, 2) + POWER(position_y - ?, 2) + POWER(position_z - ?, 2)) AS order_ FROM gameobject WHERE map = ? AND (POWER(position_x - ?, 2) + POWER(position_y - ?, 2) + POWER(position_z - ?, 2)) <= ? ORDER BY order_", CONNECTION_SYNCH);
    PrepareStatement(WORLD_SEL_CREATURE_NEAREST, "SELECT guid, id, position_x, position_y, position_z, map, (POWER(position_x - ?, 2) + POWER(position_y - ?, 2) + POWER(position_z - ?, 2)) AS order_ FROM creature WHERE map = ? AND (POWER(position_x - ?, 2) + POWER(position_y - ?, 2) + POWER(position_z - ?, 2)) <= ? ORDER BY order_", CONNECTION_SYNCH);
