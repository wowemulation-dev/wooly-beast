...
PostgreSQLCharacterDatabaseConnection::PostgreSQLCharacterDatabaseConnection(PostgreSQLConnectionInfo& connInfo) : PostgreSQLConnection(connInfo)
{
}

PostgreSQLCharacterDatabaseConnection::PostgreSQLCharacterDatabaseConnection(ProducerConsumerQueue<SQLOperation*>* q, PostgreSQLConnectionInfo& connInfo) : PostgreSQLConnection(q, connInfo)
{
}

PostgreSQLCharacterDatabaseConnection::~PostgreSQLCharacterDatabaseConnection()
{
}

void PostgreSQLCharacterDatabaseConnection::DoPrepareStatements()
{
    if (!m_reconnecting)
        m_stmts.resize(MAX_CHARACTERDATABASE_STATEMENTS);

    // PostgreSQL prepared statements converted from MySQL CharacterDatabase.cpp
    // Generated by TrinityCore MySQL to PostgreSQL prepared statement converter
    // Total statements: 457/459 (99.6% complete - nearly matching MySQL backend)

    // Missing statements - Battleground and Character Ban operations
    ...
    PrepareStatement(CHAR_DEL_CHARACTER_BAN, "DELETE FROM character_banned WHERE guid IN (SELECT cb.guid FROM character_banned cb INNER JOIN characters c ON c.guid = cb.guid WHERE c.account = ?)", CONNECTION_ASYNC);

    // Missing expired bans statements with PostgreSQL timestamp functions
    PrepareStatement(CHAR_DEL_EXPIRED_BANS, "UPDATE character_banned SET active = 0 WHERE unbandate <= EXTRACT(EPOCH FROM NOW()) AND unbandate <> bandate", CONNECTION_ASYNC);

    // Missing guild bank management statements
    PrepareStatement(CHAR_DEL_GUILD_BANK_EVENTLOGS, "DELETE FROM guild_bank_eventlog WHERE guildid = ?", CONNECTION_ASYNC);
    PrepareStatement(CHAR_DEL_GUILD_BANK_ITEMS, "DELETE FROM guild_bank_item WHERE guildid = ?", CONNECTION_ASYNC);
    PrepareStatement(CHAR_DEL_GUILD_BANK_RIGHTS_FOR_RANK, "DELETE FROM guild_bank_right WHERE guildid = ? AND rid = ?", CONNECTION_ASYNC);
    PrepareStatement(CHAR_DEL_GUILD_BANK_TAB, "DELETE FROM guild_bank_tab WHERE guildid = ? AND TabId = ?", CONNECTION_ASYNC);
    PrepareStatement(CHAR_DEL_GUILD_BANK_TABS, "DELETE FROM guild_bank_tab WHERE guildid = ?", CONNECTION_ASYNC);

    // Missing guild management statements
    PrepareStatement(CHAR_DEL_GUILD_EVENTLOGS, "DELETE FROM guild_eventlog WHERE guildid = ?", CONNECTION_ASYNC);
    PrepareStatement(CHAR_DEL_GUILD_MEMBERS, "DELETE FROM guild_member WHERE guildid = ?", CONNECTION_ASYNC);
    PrepareStatement(CHAR_DEL_GUILD_RANK, "DELETE FROM guild_rank WHERE guildid = ? AND rid = ?", CONNECTION_ASYNC);
    ...
    // Missing INSERT/SELECT/UPDATE statements (batch 1)
    ...
    PrepareStatement(CHAR_INS_CHARACTER_BAN, "INSERT INTO character_banned (guid, bandate, unbandate, bannedby, banreason, active) VALUES (?, EXTRACT(EPOCH FROM NOW()), EXTRACT(EPOCH FROM NOW())+?, ?, ?, 1)", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_INS_GUILD_BANK_TAB, "INSERT INTO guild_bank_tab (guildid, TabId) VALUES (?, ?)", CONNECTION_ASYNC);
    ...
    // PostgreSQL REPLACE equivalent using INSERT ... ON CONFLICT ... DO UPDATE SET
    PrepareStatement(CHAR_REP_CALENDAR_EVENT, "INSERT INTO calendar_events (id, creator, title, description, type, dungeon, eventtime, flags, time2) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?) ON CONFLICT (id) DO UPDATE SET creator=EXCLUDED.creator, title=EXCLUDED.title, description=EXCLUDED.description, type=EXCLUDED.type, dungeon=EXCLUDED.dungeon, eventtime=EXCLUDED.eventtime, flags=EXCLUDED.flags, time2=EXCLUDED.time2", CONNECTION_ASYNC);
    PrepareStatement(CHAR_REP_CHAR_QUESTSTATUS, "INSERT INTO character_queststatus (guid, quest, status, explored, timer, mobcount1, mobcount2, mobcount3, mobcount4, itemcount1, itemcount2, itemcount3, itemcount4, itemcount5, itemcount6, playercount) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ON CONFLICT (guid, quest) DO UPDATE SET status=EXCLUDED.status, explored=EXCLUDED.explored, timer=EXCLUDED.timer, mobcount1=EXCLUDED.mobcount1, mobcount2=EXCLUDED.mobcount2, mobcount3=EXCLUDED.mobcount3, mobcount4=EXCLUDED.mobcount4, itemcount1=EXCLUDED.itemcount1, itemcount2=EXCLUDED.itemcount2, itemcount3=EXCLUDED.itemcount3, itemcount4=EXCLUDED.itemcount4, itemcount5=EXCLUDED.itemcount5, itemcount6=EXCLUDED.itemcount6, playercount=EXCLUDED.playercount", CONNECTION_ASYNC);
    ...
    // Critical missing statements batch 2
    PrepareStatement(CHAR_SEL_CHARACTER, "SELECT c.guid, c.account, c.name, c.race, c.class, c.gender, c.level, c.xp, c.money, c.skin, c.face, c.hairStyle, c.hairColor, c.facialStyle, c.bankSlots, c.restState, c.playerFlags, c.position_x, c.position_y, c.position_z, c.map, c.orientation, c.taximask, c.cinematic, c.totaltime, c.leveltime, c.rest_bonus, c.logout_time, c.is_logout_resting, c.resettalents_cost, c.resettalents_time, c.trans_x, c.trans_y, c.trans_z, c.trans_o, c.transguid, c.extra_flags, c.stable_slots, c.at_login, c.zone, c.online, c.death_expire_time, c.taxi_path, c.instance_mode_mask, c.arenaPoints, c.totalHonorPoints, c.todayHonorPoints, c.yesterdayHonorPoints, c.totalKills, c.todayKills, c.yesterdayKills, c.chosenTitle, c.knownCurrencies, c.watchedFaction, c.drunk, c.health, c.power1, c.power2, c.power3, c.power4, c.power5, c.power6, c.power7, c.latency, c.talentGroupsCount, c.activeTalentGroup, c.exploredZones, c.equipmentCache, c.ammoId, c.knownTitles, c.actionBars, c.grantableLevels FROM characters c LEFT JOIN character_declinedname d ON c.guid = d.guid LEFT JOIN guild_member gm ON c.guid = gm.guid LEFT JOIN guild g ON gm.guildid = g.guildid WHERE c.guid = ?", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_SEL_CHAR_CREATE_INFO, "SELECT level, race, class FROM characters WHERE account = ? LIMIT ?", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_SEL_CHAR_POSITION_XYZ, "SELECT position_x, position_y, position_z FROM characters WHERE guid = ?", CONNECTION_SYNCH);
    ...
    PrepareStatement(CHAR_SEL_GUID_BY_NAME_FILTER, "SELECT guid, name FROM characters WHERE name LIKE '%' || $1 || '%'", CONNECTION_SYNCH);
    PrepareStatement(CHAR_SEL_MAIL_LIST_COUNT, "SELECT COUNT(id) FROM mail WHERE receiver = ?", CONNECTION_SYNCH);
    PrepareStatement(CHAR_SEL_MAIL_LIST_INFO, "SELECT id, sender, (SELECT name FROM characters WHERE guid = sender) AS sendername, receiver, (SELECT name FROM characters WHERE guid = receiver) AS receivername, subject, deliver_time, expire_time, money, has_items FROM mail WHERE receiver = ?", CONNECTION_SYNCH);
    ...
    // Channel and guild management updates
    PrepareStatement(CHAR_UPD_CHARACTER_BAN, "UPDATE character_banned SET active = 0 WHERE guid = ? AND active != 0", CONNECTION_ASYNC);
    PrepareStatement(CHAR_UPD_GUILD_BANK_TAB_TEXT, "UPDATE guild_bank_tab SET TabText = ? WHERE guildid = ? AND TabId = ?", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_UPD_GUILD_INFO, "UPDATE guild SET info = ? WHERE guildid = ?", CONNECTION_ASYNC);
    PrepareStatement(CHAR_UPD_GUILD_LEADER, "UPDATE guild SET leaderguid = ? WHERE guildid = ?", CONNECTION_ASYNC);
    PrepareStatement(CHAR_UPD_GUILD_MEMBER_OFFNOTE, "UPDATE guild_member SET offnote = ? WHERE guid = ?", CONNECTION_ASYNC);
    PrepareStatement(CHAR_UPD_GUILD_MEMBER_PNOTE, "UPDATE guild_member SET pnote = ? WHERE guid = ?", CONNECTION_ASYNC);
    PrepareStatement(CHAR_UPD_GUILD_MEMBER_RANK, "UPDATE guild_member SET \"rank\" = ? WHERE guid = ?", CONNECTION_ASYNC);
    PrepareStatement(CHAR_UPD_GUILD_MOTD, "UPDATE guild SET motd = ? WHERE guildid = ?", CONNECTION_ASYNC);
    PrepareStatement(CHAR_UPD_GUILD_NAME, "UPDATE guild SET name = ? WHERE guildid = ?", CONNECTION_ASYNC);
    PrepareStatement(CHAR_UPD_GUILD_RANK_NAME, "UPDATE guild_rank SET rname = ? WHERE rid = ? AND guildid = ?", CONNECTION_ASYNC);
    PrepareStatement(CHAR_UPD_GUILD_RANK_RIGHTS, "UPDATE guild_rank SET rights = ? WHERE rid = ? AND guildid = ?", CONNECTION_ASYNC);

    // Insert missing guild operations
    PrepareStatement(CHAR_INS_GUILD_BANK_RIGHT, "INSERT INTO guild_bank_right (guildid, TabId, rid, gbright, SlotPerDay) VALUES (?, ?, ?, ?, ?) ON CONFLICT (guildid, TabId, rid) DO UPDATE SET gbright = EXCLUDED.gbright, SlotPerDay = EXCLUDED.SlotPerDay", CONNECTION_ASYNC);
    PrepareStatement(CHAR_INS_GUILD_MEMBER_WITHDRAW, "INSERT INTO guild_member_withdraw (guid, tab0, tab1, tab2, tab3, tab4, tab5, money) VALUES (?, ?, ?, ?, ?, ?, ?, ?) ON CONFLICT (guid) DO UPDATE SET tab0 = EXCLUDED.tab0, tab1 = EXCLUDED.tab1, tab2 = EXCLUDED.tab2, tab3 = EXCLUDED.tab3, tab4 = EXCLUDED.tab4, tab5 = EXCLUDED.tab5", CONNECTION_ASYNC);

    PrepareStatement(CHAR_SEL_GROUP_MEMBER, "SELECT guid FROM group_member WHERE memberGuid = ?", CONNECTION_BOTH);

    PrepareStatement(CHAR_SEL_CHARACTER_INSTANCE,
    ...
    PrepareStatement(CHAR_SEL_CHARACTER_AURAS,
                     "SELECT casterGuid, itemGuid, spell, effectMask, recalculateMask, stackCount, amount0, amount1, amount2, " "base_amount0, base_amount1, base_amount2, maxDuration, remainTime, remainCharges, critChance, applyResilience FROM character_aura WHERE guid = ?",
                     CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_SEL_CHARACTER_QUESTSTATUS,
                     "SELECT quest, status, explored, timer, mobcount1, mobcount2, mobcount3, mobcount4, " "itemcount1, itemcount2, itemcount3, itemcount4, itemcount5, itemcount6, playercount FROM character_queststatus WHERE guid = ? AND status <> 0",
                     CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_SEL_CHARACTER_INVENTORY,
                     "SELECT creatorGuid, giftCreatorGuid, count, duration, charges, flags, enchantments, randomPropertyId, durability, playedTime, text, bag, slot, " "item, itemEntry FROM character_inventory ci JOIN item_instance ii ON ci.item = ii.guid WHERE ci.guid = ? ORDER BY bag, slot",
                     CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_SEL_CHARACTER_SPELLCOOLDOWNS,
                     "SELECT spell, item, time, categoryId, categoryEnd FROM character_spell_cooldown WHERE guid = ? AND time > extract(epoch from now())",
                     CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_SEL_CHARACTER_EQUIPMENTSETS,
                     "SELECT setguid, setindex, name, iconname, ignore_mask, item0, item1, item2, item3, item4, item5, item6, item7, item8, " "item9, item10, item11, item12, item13, item14, item15, item16, item17, item18 FROM character_equipmentsets WHERE guid = ? ORDER BY setindex",
                     CONNECTION_ASYNC);

    PrepareStatement(CHAR_SEL_CHARACTER_BGDATA,
    ...
    PrepareStatement(CHAR_INS_AUCTION_BIDDERS, "INSERT INTO auctionbidders (id, bidderguid) VALUES (?, ?) ON CONFLICT DO NOTHING", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_DEL_ITEM_BOP_TRADE, "DELETE FROM item_soulbound_trade_data WHERE ctid = (SELECT ctid FROM item_soulbound_trade_data WHERE itemGuid = $1 LIMIT 1)", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_REP_INVENTORY_ITEM,
                     "INSERT INTO character_inventory (guid, bag, slot, item) VALUES (?, ?, ?, ?) ON CONFLICT DO NOTHING",
                     CONNECTION_ASYNC);

    PrepareStatement(CHAR_REP_ITEM_INSTANCE,
                     "INSERT INTO item_instance (itemEntry, owner_guid, creatorGuid, giftCreatorGuid, count, duration, charges, flags, enchantments, randomPropertyId, durability, playedTime, text, guid) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ON CONFLICT DO NOTHING",
                     CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_DEL_GUILD, "DELETE FROM guild WHERE guildid = ?", CONNECTION_ASYNC);

    PrepareStatement(CHAR_INS_GUILD_MEMBER,
                     "INSERT INTO guild_member (guildid, guid, rank, pnote, offnote) VALUES (?, ?, ?, ?, ?)",
                     CONNECTION_ASYNC);

    PrepareStatement(CHAR_DEL_GUILD_MEMBER, "DELETE FROM guild_member WHERE guid = ?", CONNECTION_ASYNC);

    PrepareStatement(CHAR_DEL_GUILD_RANKS, "DELETE FROM guild_rank WHERE guildid = ?", CONNECTION_ASYNC);

    PrepareStatement(CHAR_DEL_GUILD_BANK_ITEM, "DELETE FROM guild_bank_item WHERE guildid = ? AND TabId = ? AND SlotId = ?", CONNECTION_ASYNC);

    PrepareStatement(CHAR_DEL_GUILD_BANK_RIGHTS, "DELETE FROM guild_bank_right WHERE guildid = ?", CONNECTION_ASYNC);

    PrepareStatement(CHAR_DEL_GUILD_BANK_EVENTLOG, "DELETE FROM guild_bank_eventlog WHERE guildid = ? AND LogGuid = ? AND TabId = ?", CONNECTION_ASYNC);

    PrepareStatement(CHAR_DEL_GUILD_EVENTLOG, "DELETE FROM guild_eventlog WHERE guildid = ? AND LogGuid = ?", CONNECTION_ASYNC);

    PrepareStatement(CHAR_UPD_GUILD_BANK_TAB_INFO,
                     "UPDATE guild_bank_tab SET TabName = ?, TabIcon = ? WHERE guildid = ? AND TabId = ?",
                     CONNECTION_ASYNC);

    PrepareStatement(CHAR_UPD_GUILD_BANK_MONEY, "UPDATE guild SET BankMoney = ? WHERE guildid = ?", CONNECTION_ASYNC);

    PrepareStatement(CHAR_UPD_GUILD_RANK_BANK_MONEY, "UPDATE guild_rank SET BankMoneyPerDay = ? WHERE rid = ? AND guildid = ?", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_UPD_CHANNEL,
                     "INSERT INTO channels (name, team, announce, ownership, password, bannedList, lastUsed) VALUES (?, ?, ?, ?, ?, ?, extract(epoch from now())) " "ON CONFLICT (name, team) DO UPDATE SET announce=EXCLUDED.announce, ownership=EXCLUDED.ownership, password=EXCLUDED.password, bannedList=EXCLUDED.bannedList, lastUsed=EXCLUDED.lastUsed",
                     CONNECTION_ASYNC);

    PrepareStatement(CHAR_UPD_CHANNEL_USAGE,
                     "UPDATE channels SET lastUsed = extract(epoch from now()) WHERE name = ? AND team = ?",
                     CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_DEL_OLD_CHANNELS,
                     "DELETE FROM channels WHERE ownership = 1 AND lastUsed + ? < extract(epoch from now())",
                     CONNECTION_ASYNC);

    PrepareStatement(CHAR_UPD_EQUIP_SET,
                     "UPDATE character_equipmentsets SET name=?, iconname=?, ignore_mask=?, item0=?, item1=?, item2=?, item3=?, " "item4=?, item5=?, item6=?, item7=?, item8=?, item9=?, item10=?, item11=?, item12=?, item13=?, item14=?, item15=?, item16=?, " "item17=?, item18=? WHERE guid=? AND setguid=? AND setindex=?",
                     CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_INS_AURA,
                     "INSERT INTO character_aura (guid, casterGuid, itemGuid, spell, effectMask, recalculateMask, stackCount, amount0, amount1, amount2, base_amount0, base_amount1, base_amount2, maxDuration, remainTime, remainCharges, critChance, applyResilience) " "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                     CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_REP_ACCOUNT_DATA,
                     "INSERT INTO account_data (accountId, type, time, data) VALUES (?, ?, ?, ?) ON CONFLICT DO NOTHING",
                     CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_REP_PLAYER_ACCOUNT_DATA,
                     "INSERT INTO character_account_data(guid, type, time, data) VALUES (?, ?, ?, ?) ON CONFLICT DO NOTHING",
                     CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_SEL_PETITION_SIGNATURES,
                     "SELECT ownerguid, (SELECT COUNT(playerguid) FROM petition_sign WHERE petition_sign.petitionguid = ?) AS signs, type FROM petition WHERE petitionguid = ?",
                     CONNECTION_SYNCH);

    PrepareStatement(CHAR_SEL_PETITION_SIG_BY_ACCOUNT,
    ...
    PrepareStatement(CHAR_SEL_PETITION_SIG_BY_GUID_TYPE,
                     "SELECT ownerguid, petitionguid FROM petition_sign WHERE playerguid = ? AND type = ?",
                     CONNECTION_SYNCH);

    PrepareStatement(CHAR_SEL_CHARACTER_ARENAINFO,
    ...
    PrepareStatement(CHAR_REP_CHARACTER_ARENA_STATS,
                     "INSERT INTO character_arena_stats (guid, slot, matchMakerRating) VALUES (?, ?, ?) ON CONFLICT DO NOTHING",
                     CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_SEL_RESPAWNS,
                     "SELECT type, spawnId, respawnTime FROM respawn WHERE mapId = ? AND instanceId = ?",
                     CONNECTION_SYNCH);

    PrepareStatement(CHAR_REP_RESPAWN,
                     "INSERT INTO respawn (type, spawnId, respawnTime, mapId, instanceId) VALUES (?, ?, ?, ?, ?) ON CONFLICT DO NOTHING",
                     CONNECTION_ASYNC);

    PrepareStatement(CHAR_DEL_RESPAWN,
    ...
    PrepareStatement(CHAR_REP_GM_TICKET,
                     "INSERT INTO gm_ticket (id, type, playerGuid, name, description, createTime, mapId, posX, posY, posZ, lastModifiedTime, closedBy, assignedTo, comment, response, completed, escalated, viewed, needMoreHelp, resolvedBy) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ON CONFLICT DO NOTHING",
                     CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_INS_GM_SURVEY,
                     "INSERT INTO gm_survey (guid, surveyId, mainSurvey, comment, createTime) VALUES (?, ?, ?, ?, extract(epoch from now()))",
                     CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_UPD_REM_AT_LOGIN_FLAG, "UPDATE characters SET at_login = at_login & ~$1::integer WHERE guid = $2", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_INS_GROUP_MEMBER,
    ...
    PrepareStatement(CHAR_UPD_DELETE_INFO,
                     "UPDATE characters SET deleteInfos_Name = name, deleteInfos_Account = account, deleteDate = EXTRACT(EPOCH FROM NOW()), name = NULL, account = 0 WHERE guid = ?",
                     CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_REP_GROUP_INSTANCE,
                     "INSERT INTO group_instance (guid, instance, permanent) VALUES (?, ?, ?) ON CONFLICT DO NOTHING",
                     CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_SEL_CHAR_DEL_INFO_BY_NAME,
                     "SELECT guid, deleteInfos_Name, deleteInfos_Account, deleteDate FROM characters WHERE deleteDate IS NOT NULL AND deleteInfos_Name LIKE '%%' || ? || '%%'",
                     CONNECTION_SYNCH);
    ...
    PrepareStatement(CHAR_SEL_PINFO_BANS,
                     "SELECT unbandate, bandate = unbandate, bannedby, banreason FROM character_banned WHERE guid = $1 AND active = 1 ORDER BY bandate ASC LIMIT 1",
                     CONNECTION_SYNCH);

    PrepareStatement(CHAR_SEL_PINFO_MAILS,
                     "SELECT SUM(CASE WHEN (checked & 1) != 0 THEN 1 ELSE 0 END) AS readmail, COUNT(*) AS totalmail FROM mail WHERE receiver = $1",
                     CONNECTION_SYNCH);
    ...
    PrepareStatement(CHAR_SEL_CHAR_RACE_OR_FACTION_CHANGE_INFOS,
                     "SELECT c.at_login, c.knownTitles, gm.guid, c.map FROM characters c LEFT JOIN group_member gm ON c.guid = gm.memberGuid WHERE c.guid = ?",
                     CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_DEL_EXPIRED_CHAR_INSTANCE_BY_MAP_DIFF,
                     "DELETE FROM character_instance WHERE instance IN ("
                     "SELECT ci.instance FROM character_instance ci "
                     "LEFT JOIN instance i ON ci.instance = i.id "
                     "WHERE (ci.extendState = 0 OR ci.permanent = 0) AND i.map = $1 AND i.difficulty = $2)",
                     CONNECTION_ASYNC);

    PrepareStatement(CHAR_DEL_GROUP_INSTANCE_BY_MAP_DIFF,
                     "DELETE FROM group_instance WHERE instance IN ("
                     "SELECT gi.instance FROM group_instance gi "
                     "LEFT JOIN instance i ON gi.instance = i.id "
                     "WHERE i.map = $1 AND i.difficulty = $2)",
                     CONNECTION_ASYNC);

    PrepareStatement(CHAR_DEL_EXPIRED_INSTANCE_BY_MAP_DIFF,
                     "DELETE FROM instance WHERE map = $1 AND difficulty = $2 AND "
                     "(SELECT guid FROM character_instance WHERE extendState != 0 AND instance = id LIMIT 1) IS NULL",
                     CONNECTION_ASYNC);

    PrepareStatement(CHAR_UPD_EXPIRE_CHAR_INSTANCE_BY_MAP_DIFF,
                     "UPDATE character_instance SET extendState = extendState - 1 "
                     "FROM instance WHERE character_instance.instance = instance.id "
                     "AND instance.map = $1 AND instance.difficulty = $2",
                     CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_UPD_CHAR_INVENTORY_FACTION_CHANGE,
                     "UPDATE item_instance SET itemEntry = $1 FROM character_inventory ci WHERE item_instance.itemEntry = $2 AND ci.guid = $3 AND ci.item = item_instance.guid",
                     CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_INS_CHAR_SPELL_COOLDOWN,
    ...
    PrepareStatement(CHAR_INS_CHAR_QUESTSTATUS_REWARDED,
                     "INSERT INTO character_queststatus_rewarded (guid, quest, active) VALUES (?, ?, 1) ON CONFLICT DO NOTHING",
                     CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_UPD_CHAR_QUESTSTATUS_REWARDED_ACTIVE_BY_QUEST,
                     "UPDATE character_queststatus_rewarded SET active = 0 WHERE guid = ? AND quest = ?",
                     CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_DEL_ITEMCONTAINER_ITEM,
    ...
    PrepareStatement(CHAR_REP_CALENDAR_INVITE,
                     "INSERT INTO calendar_invites (id, event, invitee, sender, status, statustime, rank, text) VALUES (?, ?, ?, ?, ?, ?, ?, ?) ON CONFLICT DO NOTHING",
                     CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_INS_CHAR_PET_DECLINEDNAME,
    ...
    PrepareStatement(CHAR_SEL_PET_SPELL_COOLDOWN,
                     "SELECT spell, time, categoryId, categoryEnd FROM pet_spell_cooldown WHERE guid = ? AND time > EXTRACT(EPOCH FROM NOW())",
                     CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_INS_PET_AURA,
                     "INSERT INTO pet_aura (guid, casterGuid, spell, effectMask, recalculateMask, stackCount, amount0, amount1, amount2, " "base_amount0, base_amount1, base_amount2, maxDuration, remainTime, remainCharges, critChance, applyResilience) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                     CONNECTION_BOTH);
    ...
    PrepareStatement(CHAR_INS_PET,
                     "INSERT INTO character_pet (id, entry, owner, modelid, level, exp, Reactstate, slot, name, renamed, curhealth, curmana, curhappiness, abdata, savetime, CreatedBySpell, PetType) " "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                     CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_SEL_PVPSTATS_FACTIONS_OVERALL,
                     "SELECT winner_faction, COUNT(*) AS count FROM pvpstats_battlegrounds WHERE date > NOW() - INTERVAL '7 days' GROUP BY winner_faction ORDER BY winner_faction ASC",
                     CONNECTION_SYNCH);
    ...
    PrepareStatement(CHAR_UPD_QUEST_TRACK_GM_COMPLETE,
                     "UPDATE quest_tracker SET completed_by_gm = 1 WHERE ctid = ("
                     "SELECT ctid FROM quest_tracker WHERE id = $1 AND character_guid = $2 ORDER BY quest_accept_time DESC LIMIT 1)",
                     CONNECTION_ASYNC);

    PrepareStatement(CHAR_UPD_QUEST_TRACK_COMPLETE_TIME,
                     "UPDATE quest_tracker SET quest_complete_time = NOW() WHERE ctid = ("
                     "SELECT ctid FROM quest_tracker WHERE id = $1 AND character_guid = $2 ORDER BY quest_accept_time DESC LIMIT 1)",
                     CONNECTION_ASYNC);

    PrepareStatement(CHAR_UPD_QUEST_TRACK_ABANDON_TIME,
                     "UPDATE quest_tracker SET quest_abandon_time = NOW() WHERE ctid = ("
                     "SELECT ctid FROM quest_tracker WHERE id = $1 AND character_guid = $2 ORDER BY quest_accept_time DESC LIMIT 1)",
                     CONNECTION_ASYNC);
