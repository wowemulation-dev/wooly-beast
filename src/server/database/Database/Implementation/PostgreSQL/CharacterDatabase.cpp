...
#define SelectItemInstanceContent "ii.guid, ii.itemEntry, ii.creatorGuid, ii.giftCreatorGuid, ii.count, ii.duration, ii.charges, ii.flags, ii.enchantments, ii.randomBonusListId, " \
    ...
    PrepareStatement(CHAR_DEL_EXPIRED_BANS, "UPDATE character_banned SET active = 0 WHERE unbandate <= EXTRACT(EPOCH FROM NOW())::BIGINT AND unbandate <> bandate", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_SEL_CHAR_CREATE_INFO, "SELECT level, race, class FROM characters WHERE account = ? LIMIT ? OFFSET 0", CONNECTION_ASYNC);
    PrepareStatement(CHAR_INS_CHARACTER_BAN, "INSERT INTO character_banned (guid, bandate, unbandate, bannedby, banreason, active) VALUES (?, EXTRACT(EPOCH FROM NOW())::BIGINT, EXTRACT(EPOCH FROM NOW())::BIGINT + ?, ?, ?, 1)", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_SEL_GUID_BY_NAME_FILTER, "SELECT guid, name FROM characters WHERE name LIKE '%' || ? || '%'", CONNECTION_SYNCH);
    ...
    PrepareStatement(CHAR_SEL_CHARACTER_SPELLCOOLDOWNS, "SELECT spell, item, time, categoryId, categoryEnd FROM character_spell_cooldown WHERE guid = ? AND time > EXTRACT(EPOCH FROM NOW())::BIGINT", CONNECTION_ASYNC);
    PrepareStatement(CHAR_SEL_CHARACTER_SPELL_CHARGES, "SELECT categoryId, rechargeStart, rechargeEnd FROM character_spell_charges WHERE guid = ? AND rechargeEnd > EXTRACT(EPOCH FROM NOW())::BIGINT ORDER BY rechargeEnd", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_INS_CHARACTER_FAVORITE_AUCTION, "INSERT INTO character_favorite_auctions (guid, \"order\", itemId, itemLevel, battlePetSpeciesId, suffixItemNameDescriptionId) VALUES (?, ?, ?, ?, ?, ?)", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_DEL_AUCTION, "WITH target AS (SELECT id FROM auctionhouse WHERE id = ?), del_ai AS (DELETE FROM auction_items WHERE auctionId IN (SELECT id FROM target)), del_ab AS (DELETE FROM auction_bidders WHERE auctionId IN (SELECT id FROM target)) DELETE FROM auctionhouse WHERE id IN (SELECT id FROM target)", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_DEL_ITEM_BOP_TRADE, "DELETE FROM item_soulbound_trade_data WHERE ctid = (SELECT ctid FROM item_soulbound_trade_data WHERE itemGuid = ? LIMIT 1)", CONNECTION_ASYNC);
    PrepareStatement(CHAR_INS_ITEM_BOP_TRADE, "INSERT INTO item_soulbound_trade_data VALUES (?, ?)", CONNECTION_ASYNC);
    PrepareStatement(CHAR_REP_INVENTORY_ITEM, "INSERT INTO character_inventory (guid, bag, slot, item) VALUES (?, ?, ?, ?) ON CONFLICT (guid, bag, slot) DO UPDATE SET item = EXCLUDED.item", CONNECTION_ASYNC);
    PrepareStatement(CHAR_REP_ITEM_INSTANCE, "INSERT INTO item_instance (itemEntry, owner_guid, creatorGuid, giftCreatorGuid, count, duration, charges, flags, enchantments, randomBonusListId, durability, playedTime, createTime, text, battlePetSpeciesId, battlePetBreedData, battlePetLevel, battlePetDisplayId, context, bonusListIDs, randomPropertiesId, guid) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ON CONFLICT (guid) DO UPDATE SET itemEntry = EXCLUDED.itemEntry, owner_guid = EXCLUDED.owner_guid, creatorGuid = EXCLUDED.creatorGuid, giftCreatorGuid = EXCLUDED.giftCreatorGuid, count = EXCLUDED.count, duration = EXCLUDED.duration, charges = EXCLUDED.charges, flags = EXCLUDED.flags, enchantments = EXCLUDED.enchantments, randomBonusListId = EXCLUDED.randomBonusListId, durability = EXCLUDED.durability, playedTime = EXCLUDED.playedTime, createTime = EXCLUDED.createTime, text = EXCLUDED.text, battlePetSpeciesId = EXCLUDED.battlePetSpeciesId, battlePetBreedData = EXCLUDED.battlePetBreedData, battlePetLevel = EXCLUDED.battlePetLevel, battlePetDisplayId = EXCLUDED.battlePetDisplayId, context = EXCLUDED.context, bonusListIDs = EXCLUDED.bonusListIDs, randomPropertiesId = EXCLUDED.randomPropertiesId", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_DEL_ITEM_INSTANCE_GEMS_BY_OWNER, "DELETE FROM item_instance_gems WHERE itemGuid IN (SELECT guid FROM item_instance WHERE owner_guid = ?)", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_DEL_ITEM_INSTANCE_TRANSMOG_BY_OWNER, "DELETE FROM item_instance_transmog WHERE itemGuid IN (SELECT guid FROM item_instance WHERE owner_guid = ?)", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_DEL_ITEM_INSTANCE_MODIFIERS_BY_OWNER, "DELETE FROM item_instance_modifiers WHERE itemGuid IN (SELECT guid FROM item_instance WHERE owner_guid = ?)", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_INS_GUILD_BANK_RIGHT, "INSERT INTO guild_bank_right (guildid, TabId, rid, gbright, SlotPerDay) VALUES (?, ?, ?, ?, ?) "
                     "ON CONFLICT (guildid, TabId, rid) DO UPDATE SET gbright = EXCLUDED.gbright, SlotPerDay = EXCLUDED.SlotPerDay", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_INS_GUILD_MEMBER_WITHDRAW_TABS,
                     ...
                     "ON CONFLICT (guid) DO UPDATE SET tab0 = EXCLUDED.tab0, tab1 = EXCLUDED.tab1, tab2 = EXCLUDED.tab2, tab3 = EXCLUDED.tab3, tab4 = EXCLUDED.tab4, tab5 = EXCLUDED.tab5, tab6 = EXCLUDED.tab6, tab7 = EXCLUDED.tab7", CONNECTION_ASYNC);
    PrepareStatement(CHAR_INS_GUILD_MEMBER_WITHDRAW_MONEY, "INSERT INTO guild_member_withdraw (guid, money) VALUES (?, ?) ON CONFLICT (guid) DO UPDATE SET money = EXCLUDED.money", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_INS_GUILD_NEWS, "INSERT INTO guild_newslog (guildid, LogGuid, EventType, PlayerGuid, Flags, Value, Timestamp) VALUES (?, ?, ?, ?, ?, ?, ?)"
                     " ON CONFLICT (guildid, LogGuid) DO UPDATE SET EventType = EXCLUDED.EventType, PlayerGuid = EXCLUDED.PlayerGuid, Flags = EXCLUDED.Flags, Value = EXCLUDED.Value, Timestamp = EXCLUDED.Timestamp", CONNECTION_ASYNC);

    // Chat channel handling
    PrepareStatement(CHAR_UPD_CHANNEL, "INSERT INTO channels (name, team, announce, ownership, password, bannedList, lastUsed) VALUES (?, ?, ?, ?, ?, ?, EXTRACT(EPOCH FROM NOW())::BIGINT) "
                                       "ON CONFLICT (name, team) DO UPDATE SET announce = EXCLUDED.announce, ownership = EXCLUDED.ownership, password = EXCLUDED.password, bannedList = EXCLUDED.bannedList, lastUsed = EXCLUDED.lastUsed", CONNECTION_ASYNC);
    PrepareStatement(CHAR_UPD_CHANNEL_USAGE, "UPDATE channels SET lastUsed = EXTRACT(EPOCH FROM NOW())::BIGINT WHERE name = ? AND team = ?", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_DEL_OLD_CHANNELS, "DELETE FROM channels WHERE ownership = 1 AND lastUsed + ? < EXTRACT(EPOCH FROM NOW())::BIGINT", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_REP_PLAYER_CURRENCY, "INSERT INTO character_currency (CharacterGuid, Currency, Quantity, WeeklyQuantity, TrackedQuantity, IncreasedCapQuantity, EarnedQuantity, Flags) VALUES (?, ?, ?, ?, ?, ?, ?, ?) ON CONFLICT (CharacterGuid, Currency) DO UPDATE SET Quantity = EXCLUDED.Quantity, WeeklyQuantity = EXCLUDED.WeeklyQuantity, TrackedQuantity = EXCLUDED.TrackedQuantity, IncreasedCapQuantity = EXCLUDED.IncreasedCapQuantity, EarnedQuantity = EXCLUDED.EarnedQuantity, Flags = EXCLUDED.Flags", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_REP_ACCOUNT_DATA, "INSERT INTO account_data (accountId, type, time, data) VALUES (?, ?, ?, ?) ON CONFLICT (accountId, type) DO UPDATE SET time = EXCLUDED.time, data = EXCLUDED.data", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_REP_PLAYER_ACCOUNT_DATA, "INSERT INTO character_account_data (guid, type, time, data) VALUES (?, ?, ?, ?) ON CONFLICT (guid, type) DO UPDATE SET time = EXCLUDED.time, data = EXCLUDED.data", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_REP_CHARACTER_ARENA_STATS, "INSERT INTO character_arena_stats (guid, slot, matchMakerRating) VALUES (?, ?, ?) ON CONFLICT (guid, slot) DO UPDATE SET matchMakerRating = EXCLUDED.matchMakerRating", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_DEL_CORPSES_FROM_MAP, "WITH target AS (SELECT guid FROM corpse WHERE mapId = ? AND instanceId = ?), del_cc AS (DELETE FROM corpse_customizations WHERE ownerGuid IN (SELECT guid FROM target)), del_cp AS (DELETE FROM corpse_phases WHERE OwnerGuid IN (SELECT guid FROM target)) DELETE FROM corpse WHERE guid IN (SELECT guid FROM target)", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_REP_RESPAWN, "INSERT INTO respawn (type, spawnId, respawnTime, mapId, instanceId) VALUES (?, ?, ?, ?, ?) ON CONFLICT (type, spawnId, mapId, instanceId) DO UPDATE SET respawnTime = EXCLUDED.respawnTime", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_REP_GM_BUG, "INSERT INTO gm_bug (id, playerGuid, note, createTime, mapId, posX, posY, posZ, facing, closedBy, assignedTo, comment) VALUES (?, ?, ?, EXTRACT(EPOCH FROM NOW())::BIGINT, ?, ?, ?, ?, ?, ?, ?, ?) ON CONFLICT (id) DO UPDATE SET playerGuid = EXCLUDED.playerGuid, note = EXCLUDED.note, createTime = EXCLUDED.createTime, mapId = EXCLUDED.mapId, posX = EXCLUDED.posX, posY = EXCLUDED.posY, posZ = EXCLUDED.posZ, facing = EXCLUDED.facing, closedBy = EXCLUDED.closedBy, assignedTo = EXCLUDED.assignedTo, comment = EXCLUDED.comment", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_REP_GM_COMPLAINT, "INSERT INTO gm_complaint (id, playerGuid, note, createTime, mapId, posX, posY, posZ, facing, targetCharacterGuid, reportType, reportMajorCategory, reportMinorCategoryFlags, reportLineIndex, assignedTo, closedBy, comment) VALUES (?, ?, ?, EXTRACT(EPOCH FROM NOW())::BIGINT, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ON CONFLICT (id) DO UPDATE SET playerGuid = EXCLUDED.playerGuid, note = EXCLUDED.note, createTime = EXCLUDED.createTime, mapId = EXCLUDED.mapId, posX = EXCLUDED.posX, posY = EXCLUDED.posY, posZ = EXCLUDED.posZ, facing = EXCLUDED.facing, targetCharacterGuid = EXCLUDED.targetCharacterGuid, reportType = EXCLUDED.reportType, reportMajorCategory = EXCLUDED.reportMajorCategory, reportMinorCategoryFlags = EXCLUDED.reportMinorCategoryFlags, reportLineIndex = EXCLUDED.reportLineIndex, assignedTo = EXCLUDED.assignedTo, closedBy = EXCLUDED.closedBy, comment = EXCLUDED.comment", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_REP_GM_SUGGESTION, "INSERT INTO gm_suggestion (id, playerGuid, note, createTime, mapId, posX, posY, posZ, facing, closedBy, assignedTo, comment) VALUES (?, ?, ?, EXTRACT(EPOCH FROM NOW())::BIGINT, ?, ?, ?, ?, ?, ?, ?, ?) ON CONFLICT (id) DO UPDATE SET playerGuid = EXCLUDED.playerGuid, note = EXCLUDED.note, createTime = EXCLUDED.createTime, mapId = EXCLUDED.mapId, posX = EXCLUDED.posX, posY = EXCLUDED.posY, posZ = EXCLUDED.posZ, facing = EXCLUDED.facing, closedBy = EXCLUDED.closedBy, assignedTo = EXCLUDED.assignedTo, comment = EXCLUDED.comment", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_UPD_ADD_AT_LOGIN_FLAG, "UPDATE characters SET at_login = at_login | ?::INTEGER WHERE guid = ?", CONNECTION_ASYNC);
    PrepareStatement(CHAR_UPD_REM_AT_LOGIN_FLAG, "UPDATE characters set at_login = at_login & ~ ?::INTEGER WHERE guid = ?", CONNECTION_ASYNC);
    PrepareStatement(CHAR_UPD_ALL_AT_LOGIN_FLAGS, "UPDATE characters SET at_login = at_login | ?::INTEGER", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_UPD_DELETE_INFO, "UPDATE characters SET deleteInfos_Name = name, deleteInfos_Account = account, deleteDate = EXTRACT(EPOCH FROM NOW())::BIGINT, name = NULL, account = 0 WHERE guid = ?", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_REP_WORLD_STATE, "INSERT INTO world_state_value (Id, Value) VALUES (?, ?) ON CONFLICT (Id) DO UPDATE SET Value = EXCLUDED.Value", CONNECTION_ASYNC);
    PrepareStatement(CHAR_REP_WORLD_VARIABLE, "INSERT INTO world_variable (Id, Value) VALUES (?, ?) ON CONFLICT (Id) DO UPDATE SET Value = EXCLUDED.Value", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_SEL_CHAR_DEL_INFO_BY_NAME, "SELECT guid, deleteInfos_Name, deleteInfos_Account, deleteDate FROM characters WHERE deleteDate IS NOT NULL AND deleteInfos_Name LIKE '%' || ? || '%'", CONNECTION_SYNCH);
    ...
    PrepareStatement(CHAR_SEL_PINFO_BANS, "SELECT unbandate, bandate = unbandate, bannedby, banreason FROM character_banned WHERE guid = ? AND active = 1 ORDER BY bandate ASC LIMIT 1", CONNECTION_SYNCH);
    //0: lowGUID
    ...
    PrepareStatement(CHAR_UPD_CHAR_RACE, "UPDATE characters SET race = ?, extra_flags = extra_flags | ?::INTEGER WHERE guid = ?", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_UPD_CHAR_INVENTORY_FACTION_CHANGE, "UPDATE item_instance SET itemEntry = ? FROM character_inventory ci WHERE item_instance.itemEntry = ? AND ci.guid = ? AND ci.item = item_instance.guid", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_REP_CHAR_QUESTSTATUS, "INSERT INTO character_queststatus (guid, quest, status, explored, acceptTime, endTime) VALUES (?, ?, ?, ?, ?, ?) ON CONFLICT (guid, quest) DO UPDATE SET status = EXCLUDED.status, explored = EXCLUDED.explored, acceptTime = EXCLUDED.acceptTime, endTime = EXCLUDED.endTime", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_REP_CHAR_QUESTSTATUS_OBJECTIVES, "INSERT INTO character_queststatus_objectives (guid, quest, objective, data) VALUES (?, ?, ?, ?) ON CONFLICT (guid, quest, objective) DO UPDATE SET data = EXCLUDED.data", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_INS_CHAR_QUESTSTATUS_REWARDED, "INSERT INTO character_queststatus_rewarded (guid, quest, active) VALUES (?, ?, 1) ON CONFLICT DO NOTHING", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_REP_CHAR_VOID_STORAGE_ITEM, "INSERT INTO character_void_storage (itemId, playerGuid, itemEntry, slot, creatorGuid, randomBonusListId, fixedScalingLevel, artifactKnowledgeLevel, context, bonusListIDs) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ON CONFLICT (itemId) DO UPDATE SET playerGuid = EXCLUDED.playerGuid, itemEntry = EXCLUDED.itemEntry, slot = EXCLUDED.slot, creatorGuid = EXCLUDED.creatorGuid, randomBonusListId = EXCLUDED.randomBonusListId, fixedScalingLevel = EXCLUDED.fixedScalingLevel, artifactKnowledgeLevel = EXCLUDED.artifactKnowledgeLevel, context = EXCLUDED.context, bonusListIDs = EXCLUDED.bonusListIDs", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_REP_CHAR_CUF_PROFILES, "INSERT INTO character_cuf_profiles (guid, id, name, frameHeight, frameWidth, sortBy, healthText, boolOptions, topPoint, bottomPoint, leftPoint, topOffset, bottomOffset, leftOffset) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ON CONFLICT (guid, id) DO UPDATE SET name = EXCLUDED.name, frameHeight = EXCLUDED.frameHeight, frameWidth = EXCLUDED.frameWidth, sortBy = EXCLUDED.sortBy, healthText = EXCLUDED.healthText, boolOptions = EXCLUDED.boolOptions, topPoint = EXCLUDED.topPoint, bottomPoint = EXCLUDED.bottomPoint, leftPoint = EXCLUDED.leftPoint, topOffset = EXCLUDED.topOffset, bottomOffset = EXCLUDED.bottomOffset, leftOffset = EXCLUDED.leftOffset", CONNECTION_ASYNC);
    ...
    // Calendar
    PrepareStatement(CHAR_REP_CALENDAR_EVENT, "INSERT INTO calendar_events (EventID, Owner, Title, Description, EventType, TextureID, Date, Flags, LockDate) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?) ON CONFLICT (EventID) DO UPDATE SET Owner = EXCLUDED.Owner, Title = EXCLUDED.Title, Description = EXCLUDED.Description, EventType = EXCLUDED.EventType, TextureID = EXCLUDED.TextureID, Date = EXCLUDED.Date, Flags = EXCLUDED.Flags, LockDate = EXCLUDED.LockDate", CONNECTION_ASYNC);
    PrepareStatement(CHAR_DEL_CALENDAR_EVENT, "DELETE FROM calendar_events WHERE EventID = ?", CONNECTION_ASYNC);
    PrepareStatement(CHAR_REP_CALENDAR_INVITE, "INSERT INTO calendar_invites (InviteID, EventID, Invitee, Sender, Status, ResponseTime, ModerationRank, Note) VALUES (?, ?, ?, ?, ?, ?, ?, ?) ON CONFLICT (InviteID) DO UPDATE SET EventID = EXCLUDED.EventID, Invitee = EXCLUDED.Invitee, Sender = EXCLUDED.Sender, Status = EXCLUDED.Status, ResponseTime = EXCLUDED.ResponseTime, ModerationRank = EXCLUDED.ModerationRank, Note = EXCLUDED.Note", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_SEL_PET_SPELL_COOLDOWN, "SELECT spell, time, categoryId, categoryEnd FROM pet_spell_cooldown WHERE guid = ? AND time > EXTRACT(EPOCH FROM NOW())::BIGINT", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_SEL_PET_SPELL_CHARGES, "SELECT categoryId, rechargeStart, rechargeEnd FROM pet_spell_charges WHERE guid = ? AND rechargeEnd > EXTRACT(EPOCH FROM NOW())::BIGINT ORDER BY rechargeEnd", CONNECTION_ASYNC);
    ...
    PrepareStatement(CHAR_SEL_PVPSTATS_FACTIONS_OVERALL, "SELECT winner_faction, COUNT(*) AS count FROM pvpstats_battlegrounds WHERE date > NOW() - INTERVAL '7 days' GROUP BY winner_faction ORDER BY winner_faction ASC", CONNECTION_SYNCH);
    ...
    PrepareStatement(CHAR_UPD_QUEST_TRACK_GM_COMPLETE, "UPDATE quest_tracker SET completed_by_gm = 1 WHERE ctid = (SELECT ctid FROM quest_tracker WHERE id = ? AND character_guid = ? ORDER BY quest_accept_time DESC LIMIT 1)", CONNECTION_ASYNC);
    PrepareStatement(CHAR_UPD_QUEST_TRACK_COMPLETE_TIME, "UPDATE quest_tracker SET quest_complete_time = NOW() WHERE ctid = (SELECT ctid FROM quest_tracker WHERE id = ? AND character_guid = ? ORDER BY quest_accept_time DESC LIMIT 1)", CONNECTION_ASYNC);
    PrepareStatement(CHAR_UPD_QUEST_TRACK_ABANDON_TIME, "UPDATE quest_tracker SET quest_abandon_time = NOW() WHERE ctid = (SELECT ctid FROM quest_tracker WHERE id = ? AND character_guid = ? ORDER BY quest_accept_time DESC LIMIT 1)", CONNECTION_ASYNC);
    ...
    // War mode
    PrepareStatement(CHAR_SEL_WAR_MODE_TUNING, "SELECT race, COUNT(guid) FROM characters WHERE ((playerFlags & ?) = ?) AND logout_time >= (EXTRACT(EPOCH FROM NOW())::BIGINT - 604800) GROUP BY race", CONNECTION_SYNCH);
...
CharacterDatabaseConnection::CharacterDatabaseConnection(PostgreSQLConnectionInfo& connInfo, ConnectionFlags connectionFlags) : PostgreSQLConnection(connInfo, connectionFlags)
{
}
