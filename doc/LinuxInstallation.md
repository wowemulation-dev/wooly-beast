...
The world database (TDB) must be downloaded separately from the
[TrinityCore releases](https://github.com/TrinityCore/TrinityCore/releases).

```bash
# Download the TDB file (check releases for latest version)
# Example: TDB_full_world_335.25101_2025_10_21.7z

# Extract and copy to sql/base/
7z x TDB_full_world_335.*.7z
cp TDB_full_world_335.*.sql /path/to/TrinityCore/sql/base/
```
...
mysql -u trinity -p trinity_world < sql/base/TDB_full_world_335.*.sql

# Apply any pending updates (if auto-update is disabled)
mysql -u trinity -p trinity_auth < sql/updates/auth/3.3.5/*.sql
mysql -u trinity -p trinity_characters < sql/updates/characters/3.3.5/*.sql
mysql -u trinity -p trinity_world < sql/updates/world/3.3.5/*.sql
```
