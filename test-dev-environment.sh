...
# cata_classic: containers trinity-cata-{mysql,postgres}-*, ports MySQL 336xx, PostgreSQL 543xx
...
POSTGRES_HOTFIX_CONTAINER="${CONTAINER_PREFIX}-postgres-hotfixes"
...
# MySQL ports: auth=33606, characters=33607, world=33608, hotfixes=33609
...
# PostgreSQL ports: auth=54366, characters=54367, world=54368, hotfixes=54369
POSTGRES_AUTH_PORT="${POSTGRES_AUTH_PORT:-54366}"
POSTGRES_CHAR_PORT="${POSTGRES_CHAR_PORT:-54367}"
POSTGRES_WORLD_PORT="${POSTGRES_WORLD_PORT:-54368}"
POSTGRES_HOTFIX_PORT="${POSTGRES_HOTFIX_PORT:-54369}"

# Build directories
BUILD_DIR="${BUILD_DIR:-build-cata}"
BUILD_DIR_POSTGRES="${BUILD_DIR_POSTGRES:-build-cata-postgres}"
INSTALL_DIR="${INSTALL_DIR:-$(pwd)/install-cata}"
INSTALL_DIR_POSTGRES="${INSTALL_DIR_POSTGRES:-$(pwd)/install-cata-postgres}"
...
function show_usage() {
    ...
    echo "  CONTAINER_PREFIX       Container name prefix (default: trinity-cata)"
    ...
    echo "  BUILD_DIR              MySQL build directory (default: build-cata)"
    echo "  BUILD_DIR_POSTGRES     PostgreSQL build directory (default: build-cata-postgres)"
    echo "  INSTALL_DIR            MySQL install directory (default: ./install-cata)"
    echo "  INSTALL_DIR_POSTGRES   PostgreSQL install directory (default: ./install-cata-postgres)"
    echo ""
    ...
    echo "  hotfixes   - Hotfix data database"
    echo "               MySQL: port ${MYSQL_HOTFIX_PORT}, PostgreSQL: port ${POSTGRES_HOTFIX_PORT}"
    echo ""
}
...
function setup_postgres() {
    ...
    # PostgreSQL container for hotfixes database
    if ! podman container exists "${POSTGRES_HOTFIX_CONTAINER}"; then
        echo "Creating PostgreSQL container for hotfixes database (port ${POSTGRES_HOTFIX_PORT})..."
        podman run -d --name "${POSTGRES_HOTFIX_CONTAINER}" \
            -e POSTGRES_USER="${DB_USER}" \
            -e POSTGRES_PASSWORD="${DB_PASSWORD}" \
            -e POSTGRES_DB=hotfixes \
            -p "${POSTGRES_HOTFIX_PORT}:5432" \
            "postgres:${POSTGRES_VERSION}"
    else
        echo "Container ${POSTGRES_HOTFIX_CONTAINER} already exists"
        podman start "${POSTGRES_HOTFIX_CONTAINER}" 2>/dev/null || true
    fi
    ...
    echo "  Auth:       localhost:${POSTGRES_AUTH_PORT} (database: auth)"
    echo "  Characters: localhost:${POSTGRES_CHAR_PORT} (database: characters)"
    echo "  World:      localhost:${POSTGRES_WORLD_PORT} (database: world)"
    echo "  Hotfixes:   localhost:${POSTGRES_HOTFIX_PORT} (database: hotfixes)"
    echo "  User:       ${DB_USER}"
    echo "  Password:   ${DB_PASSWORD}"
}
...
function reset_mysql_databases() {
    ...
    if podman container exists "${MYSQL_AUTH_CONTAINER}"; then
        ...
        podman exec "${MYSQL_AUTH_CONTAINER}" mysql -u "${DB_USER}" -p"${DB_PASSWORD}" -e "
            ...
            CREATE DATABASE auth;" 2>/dev/null || echo "Warning: Could not reset MySQL auth database"
    fi
    ...
    if podman container exists "${MYSQL_CHAR_CONTAINER}"; then
        ...
        podman exec "${MYSQL_CHAR_CONTAINER}" mysql -u "${DB_USER}" -p"${DB_PASSWORD}" -e "
            ...
            CREATE DATABASE characters;" 2>/dev/null || echo "Warning: Could not reset MySQL characters database"
    fi
    ...
    if podman container exists "${MYSQL_WORLD_CONTAINER}"; then
        ...
        podman exec "${MYSQL_WORLD_CONTAINER}" mysql -u "${DB_USER}" -p"${DB_PASSWORD}" -e "
            ...
            CREATE DATABASE world;" 2>/dev/null || echo "Warning: Could not reset MySQL world database"
    fi

    # Reset hotfixes database
    if podman container exists "${MYSQL_HOTFIX_CONTAINER}"; then
        echo "Resetting MySQL hotfixes database..."
        podman exec "${MYSQL_HOTFIX_CONTAINER}" mysql -u "${DB_USER}" -p"${DB_PASSWORD}" -e "
            DROP DATABASE IF EXISTS hotfixes;
            CREATE DATABASE hotfixes;" 2>/dev/null || echo "Warning: Could not reset MySQL hotfixes database"
    fi
}
...
function configure() {
    ...
    sed -i "s|^LoginDatabaseInfo = .*|LoginDatabaseInfo = \"127.0.0.1;${MYSQL_AUTH_PORT};${DB_USER};${DB_PASSWORD};auth\"|" \
    ...
    sed -i "s|^WorldDatabaseInfo.*= .*|WorldDatabaseInfo     = \"127.0.0.1;${MYSQL_WORLD_PORT};${DB_USER};${DB_PASSWORD};world\"|" \
        "${INSTALL_DIR}/etc/worldserver.conf"
    sed -i "s|^CharacterDatabaseInfo.*= .*|CharacterDatabaseInfo = \"127.0.0.1;${MYSQL_CHAR_PORT};${DB_USER};${DB_PASSWORD};characters\"|" \
        "${INSTALL_DIR}/etc/worldserver.conf"
    sed -i "s|^HotfixDatabaseInfo.*= .*|HotfixDatabaseInfo    = \"127.0.0.1;${MYSQL_HOTFIX_PORT};${DB_USER};${DB_PASSWORD};hotfixes\"|" \
        "${INSTALL_DIR}/etc/worldserver.conf"
...
function configure_postgres() {
    ...
    # Copy dist files if configs don't exist
    if [ ! -f "${INSTALL_DIR_POSTGRES}/etc/bnetserver.conf" ]; then
        cp "${INSTALL_DIR_POSTGRES}/etc/bnetserver.conf.dist" "${INSTALL_DIR_POSTGRES}/etc/bnetserver.conf"
    fi
    ...
    # Update bnetserver configuration for PostgreSQL
    sed -i "s|^LoginDatabaseInfo = .*|LoginDatabaseInfo = \"127.0.0.1;${POSTGRES_AUTH_PORT};${DB_USER};${DB_PASSWORD};auth\"|" \
        "${INSTALL_DIR_POSTGRES}/etc/bnetserver.conf"
    sed -i 's|^Updates.EnableDatabases = .*|Updates.EnableDatabases = 1|' \
        "${INSTALL_DIR_POSTGRES}/etc/bnetserver.conf"
    ...
    sed -i "s|^WorldDatabaseInfo.*= .*|WorldDatabaseInfo     = \"127.0.0.1;${POSTGRES_WORLD_PORT};${DB_USER};${DB_PASSWORD};world\"|" \
        "${INSTALL_DIR_POSTGRES}/etc/worldserver.conf"
    sed -i "s|^CharacterDatabaseInfo.*= .*|CharacterDatabaseInfo = \"127.0.0.1;${POSTGRES_CHAR_PORT};${DB_USER};${DB_PASSWORD};characters\"|" \
        "${INSTALL_DIR_POSTGRES}/etc/worldserver.conf"
    sed -i "s|^HotfixDatabaseInfo.*= .*|HotfixDatabaseInfo    = \"127.0.0.1;${POSTGRES_HOTFIX_PORT};${DB_USER};${DB_PASSWORD};hotfixes\"|" \
        "${INSTALL_DIR_POSTGRES}/etc/worldserver.conf"
...
function test_server() {
    ...
    if kill -0 "${BNET_PID}" 2>/dev/null; then
        echo "MySQL bnetserver started successfully"
        kill "${BNET_PID}" 2>/dev/null || true
        wait "${BNET_PID}" 2>/dev/null || true
        cd - >/dev/null
        return 0
    else
        echo "MySQL bnetserver failed to start"
        cd - >/dev/null
        return 1
    fi
}
...
function test_postgres() {
    ...
    echo "Starting PostgreSQL bnetserver for testing..."
    ...
    if kill -0 "${BNET_PID}" 2>/dev/null; then
        echo "PostgreSQL bnetserver started successfully"
        kill "${BNET_PID}" 2>/dev/null || true
        wait "${BNET_PID}" 2>/dev/null || true
        cd - >/dev/null
        return 0
    else
        echo "PostgreSQL bnetserver failed to start"
        cd - >/dev/null
        return 1
    fi
}
...
function show_status() {
    ...
    if [ -d "${INSTALL_DIR_POSTGRES}" ]; then
        ...
        [ -f "${INSTALL_DIR_POSTGRES}/bin/bnetserver" ] && echo "    bnetserver: found" || echo "    bnetserver: missing"
